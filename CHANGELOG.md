...
- 響應式版面整改：手機直向、手機橫向、桌面/平板分開處理。
- 手機直向固定單欄滿寬。
- 手機橫向只有 1 張可見卡時強制滿寬；2 張以上且空間足夠才切雙欄 Masonry。
- 桌面/平板只有 1 張可見卡時也改為滿寬，避免右側留下大面積空白。
- 新增 `visualViewport.resize` 與 `orientationchange` 二次延遲重算，修正 iOS 旋轉期間 viewport 尚未穩定造成的錯位。
- 保留 v62 重構架構、v61 目前事件/最近事件分離及既有資料來源邏輯。
