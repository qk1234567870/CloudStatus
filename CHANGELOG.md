...
- 全面重構前端響應式與設定架構。
- 所有雙欄版面統一使用真正 Masonry：每張新卡片放入目前較短的一欄，自動階梯補位。
- `<560px` 單欄；`>=560px` 固定雙欄階梯補位；不再區分手機橫屏、平板與桌面。
- 完整移除歷史 CSS 疊加，`style.css` 重寫為單一乾淨樣式層。
- `assets/config.js` 升級為唯一設定入口，集中所有功能開關、版面參數、刷新、快取、網路、事件數量、分類、跨境排序、狀態文字與 UI 文字。
- `index.html` 改為純結構骨架；分類與介面文字由 config 動態建立。
- Masonry 重寫為 shortest-column 演算法。
- ResizeObserver 改監看外層容器寬度，避免 Masonry 改高度造成重排迴圈。
- 保留目前事件 / 最近事件分離與全部 23 個服務資料源邏輯。
- 所有資源同步更新至 `v72.0.0`。

## v71.0.0

- 再次完整重構響應式版面，取消方向與裝置類型判斷。
- 統一成三段式純寬度模型：`<560px` 單欄、`560–1179px` 雙欄 CSS Grid、`>=1180px` 雙欄 Masonry。
- 手機旋轉不需要特殊分支；橫屏寬度自然進入雙欄，直屏自然回到單欄。
- 560–1179px 全部使用正常文流，不再存在手機橫屏 absolute 定位、容器高度錯誤、卡片重疊或下拉範圍消失問題。
- 只有完整桌面寬度才啟用 Masonry 階梯補位。
- 重構卡片 Header Grid，固定「名稱 + 來源 / 灰色副標題」結構。
- 來源名稱加入最大寬度與省略處理，避免擠壓服務名稱與副標題。
- 分類列、搜尋列、統計列與所有卡片加入一致的 min-width / overflow 邊界。
- 保留全部資料源、事件模型、Cloudflare Radar 與 23 個服務邏輯。
- 所有資源同步更新至 `v71.0.0`。

## v70.0.0

- 根據手機橫屏實機截圖再次重構版面。
- 修正橫屏卡片只顯示半邊、右側大片空白，以及副標題被壓成逐字換行。
- 手機橫屏不再使用 absolute-position Masonry，改為原生雙欄 CSS Grid。
- 手機橫屏所有服務卡片保持正常文流，因此頁面高度與上下捲動由瀏覽器原生計算。
- 卡片標題區固定為「服務名稱 / 來源」第一列、「完整灰色副標題」第二列，避免中文副標題被擠成窄直欄。
- 手機直屏維持單欄。
- 真正桌面寬度才使用雙欄 Masonry。
- 所有資源同步更新至 `v70.0.0`。
...
- 修正手機橫屏後頁面可捲動範圍 / 下拉內容異常。
- 根因：v58–v68 多代響應式 CSS 疊加，旋轉後 `grid`、`Masonry`、卡片定位與容器高度可能互相覆蓋。
- 移除舊版累積的 RWD 規則，重建單一響應式版面層。
- 啟用先前存在但未實際啟動的 `ResizeObserver`。
- 同時監聽 `resize`、`orientationchange` 與 Safari `visualViewport.resize`。
- 旋轉後執行多次穩定重排，待瀏覽器工具列與安全區尺寸確定後重新計算 Masonry 高度。
- 直屏窄畫面維持單欄；橫屏寬度足夠時維持雙欄 Masonry。
- 頁面高度由實際 Masonry 最長欄重新計算，所有服務都可正常向下捲動。
- 所有資源同步更新至 `v69.0.0`。
...
- 改為直屏 / 橫屏自適應版面。
- 手機直屏：一般寬度維持單欄。
- 手機橫屏：可用寬度 >= 560px 自動切換雙欄 Masonry。
- 極窄橫屏 < 560px 才維持單欄。
- 平板直屏 >= 760px 可使用雙欄。
- 桌面維持最多雙欄 Masonry。
- 旋轉螢幕時會重新計算卡片位置，不需重新整理頁面。
- 修正舊版單純使用單一寬度門檻造成直屏 / 橫屏體驗不一致的問題。
- 所有資源同步更新至 `v68.0.0`。

## v67.0.0

- 修正手機橫屏看起來只剩一張服務卡片的問題。
- 根因不是資料少，也不是仍被判成單欄，而是 v65 舊規則在 600–759px 保留了 `width:100%!important`。
- v66 的 Masonry 已把卡片定位成左右兩欄，但卡片寬度仍為 100%，因此左右卡片互相覆蓋，視覺上像只剩一張。
- 600px 以上現在強制卡片寬度為 `(100% - 14px) / 2`，與 JS Masonry 的兩欄計算完全一致。
- 600px 以下維持真正單欄。
- 不加入手機、UA、orientation、pointer 等裝置判斷，仍為純容器寬度 RWD。
- 所有資源同步更新至 `v67.0.0`。

## v66.0.0

- 修正 v65 部分手機橫屏仍只有單欄的問題。
- 原因：760px 是以 viewport 經驗值設定，但瀏覽器工具列、安全區與外層容器會讓 `#services` 實際寬度低於 760px。
- 保留純容器寬度架構，不加入手機、UA、方向或 pointer 判斷。
- `#services` 實際寬度 < 600px：單欄。
- `#services` 實際寬度 >= 600px：雙欄 Masonry。
- 一般手機直屏仍自然維持單欄；手機橫屏只要實際內容區達 600px 即自動雙欄。
- 所有資源同步更新至 `v66.0.0`。

## v65.0.0

- 全面重構響應式判定：取消手機 / 平板 / 桌面、`orientation`、`pointer`、User-Agent 等裝置識別。
- 改為純「實際內容容器寬度」驅動版面。
- `#services` 實際寬度 < 760px：單欄。
- `#services` 實際寬度 >= 760px：雙欄 Masonry。
- 因此手機直屏、手機橫屏、Fold、平板、分割視窗、桌面與瀏覽器縮放全部只按可用空間自動排版。
- 仍然最多雙欄，不會出現三欄 / 四欄。
- ResizeObserver 保持啟用，容器尺寸改變時自動重新排版；不再需要 orientation 專用處理。
- 分類列維持橫向滑動，統計卡維持三等分。
- 既有 v62 重構架構、v61 目前事件/最近事件分離、Cloudflare Radar 與所有資料源邏輯保持不變。
- 所有資源同步更新至 `v65.0.0`。
...
- 修正 v63 對手機橫屏的錯誤處理。
- 手機直屏：固定單欄。
- 手機橫屏：改為雙欄 Grid，自適配可用寬度，不使用桌面 Masonry。
- 電腦網頁端：維持 1180px 置中 + 雙欄 Masonry 階梯補位。
- 極窄橫屏（寬度 <= 680px）才自動退回單欄，避免卡片內容過度壓縮。
- 手機橫屏仍保留手機版間距、分類橫向滑動、統計三欄與卡片防溢出。
- 所有資源同步更新至 `v64.0.0`。

## v63.0.0

- 修正手機橫屏因可用寬度超過 720px 被誤判成桌面雙欄的問題。
- 手機直屏與手機橫屏現在都維持單欄服務卡片。
- 手機橫屏以 `pointer: coarse` + 橫向 + 高度條件識別，不影響真正電腦網頁端。
- 電腦端仍維持 1180px 置中與雙欄 Masonry。
- 分類列、搜尋、三個統計卡片在橫屏仍保持自適配。
- 所有資源同步更新至 `v63.0.0`。
