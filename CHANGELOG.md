...
- 按實機截圖恢復並統一「階梯補位」。
- `<560px` 維持單欄。
- `>=560px` 全部改為真正雙欄 Masonry，不再使用等高列 CSS Grid。
- 每加入一張服務卡片，都會即時計算左右欄高度，放入目前較短的一欄。
- 因此左側 Cloudflare 很高、右側 AWS 較短時，下一張服務會直接補到 AWS 下方，不會等待 Cloudflare 結束後才開始下一列。
- 手機橫屏、平板與桌面只要可用寬度達 560px，都採相同階梯補位規則。
- 保留單一乾淨的 RWD 層，沒有手機 / UA / orientation 特判。
- Masonry 容器高度由左右欄最大高度實時計算，維持完整上下捲動。
- 所有資源同步更新至 `v72.0.0`。
