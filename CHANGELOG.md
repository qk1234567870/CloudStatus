...
- 按照要求：只要進入雙欄，就一律使用 Masonry 階梯補位。
- `<560px`：單欄正常文流。
- `>=560px`：雙欄 Masonry，自動把下一張卡片補到目前較短的欄位。
- 手機橫屏、平板、窄桌面、完整桌面只要是雙欄，排版行為完全一致。
- 不再存在 560–1179px 使用普通 Grid、1180px 才使用 Masonry 的差異。
- 保留 v71 的乾淨響應式基礎，未重新加入裝置、UA、pointer 或 orientation 判斷。
- Masonry 容器高度仍由 JS 依最長欄實際計算，保持完整上下捲動。
- 所有資源同步更新至 `v72.0.0`。
