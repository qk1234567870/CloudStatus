...
- 重構桌面 Masonry 核心，修正重構後偶發只剩左欄、右側空白的問題。
- 改為兩階段排版：先固定兩欄最終寬度，再量實際高度並放入最短欄。
- 每次資料 render、瀏覽器 resize、手機旋轉、VisualViewport 改變、字型載入完成後都重新計算。
- Masonry 容器高度改由兩欄實際最大高度決定，避免下方說明文字穿入卡片區。
- 移除跨境卡片多餘第三行 `中國聯通 · 普通公網` 等分類文字。
- 灰色第二行維持 `(China Unicom · AS4837) 中國聯通 169 骨幹網`。
- 手機維持單欄正常文件流；桌面維持最多雙欄階梯自動補位。
- v62 Registry / Manifest / Loader、v61 目前事件邏輯及所有資料源保持不變。
- 所有資源同步更新至 `v63.0.0`。
