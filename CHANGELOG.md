...
- 專門整改橫屏；卡片模板與 23 個服務模組保持不動。
- 清除首頁歷代重複 media query，避免橫屏時舊 CSS 互相覆蓋。
- 修正整頁水平溢出造成的頁首、刷新按鈕、搜尋列與統計卡錯位。
- 橫屏 `.shell` 使用實際 viewport 內容寬度，不套桌面 1180px 限制。
- 分類列維持單行，空間不足仍由 v73 動態收入「更多」。
- 搜尋框與「只看異常」橫屏保持同一行。
- 三個統計卡橫屏保持等寬、有框、同一行。
- 低高度橫屏另有壓縮規則。
- 設備旋轉後仍由 ResizeObserver / orientationchange 自動重新計算分類。
