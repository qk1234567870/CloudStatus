...
- 亞洲顯示 `DC5`（Singapore）。
- 歐洲顯示 `DC2 / DC4`（Amsterdam）。
- 北美顯示 `DC1 / DC3`（Miami）。
- 南美、大洋洲、非洲顯示 `無主 DC`，表示目前 DC1–DC5 的已知主機房所在地不在這些洲。
- DC1–DC5 本地檢查列新增實體地點：邁阿密 / 阿姆斯特丹 / 新加坡。
- 每個洲仍然實測 DC1–DC5 全部端點；洲旁 DC 標示只表示主 DC 實體所在地，不代表使用者固定歸屬。
- 保留 v87 全洲自動選探針、v86 診斷、v81 GitHub Pages Actions。
- 快取更新為 `cloudstatus-cache-v88`。

## v87.0.0

- 重構 Telegram 全球探針洲級選點。
- 不再只看少數固定國家；每一洲會掃描該洲所有可識別的 Check-Host 機器節點。
- 優先國家沒有節點時，自動改選同一洲其他國家的可用節點。
- 亞洲、歐洲、北美、南美、大洋洲、非洲全部套用洲級候選池。
- 新增 `availableNodes`，顯示該洲本輪可用候選節點數。
- 整洲真的沒有可用節點時，顯示「本輪無可用節點」並附上原因。
- 不跨洲借探針，不使用使用者回報，也不把單一路徑失敗宣告成 Telegram 官方故障。
- 保留 v86 探針診斷與 GitHub Actions 自動實測。
- 快取更新為 `cloudstatus-cache-v87`。
